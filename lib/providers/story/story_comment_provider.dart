...
class StoryCommentProvider extends ChangeNotifier {
  RefreshController refreshController = RefreshController();
  int _maxPages = 0;
  int _currentPage = 0;
  bool _isLoading = false;
  List<StoryComment> _list = [];
  int _currentStory = 0;
  ...
  List<StoryComment> get list => _list;

  set list(List<StoryComment> value) {
    _list = value;
    notifyListeners();
  }

  send(StoryComment comment) async {
    isSending = true;
    JApiService apiService = JApiService();
    var response = await apiService.postRequest(JApi.ADD_STORY_COMMENT, {
    ...
    if (response != null) {
      try {
        StoryComment newComment = StoryComment.fromJson(response);
  ...
  replyComment(
      StoryComment comment, StoryComment replyComment, comment_id) async {
    isSending = true;
    JApiService apiService = JApiService();
    var response = await apiService.postRequest(JApi.REPLY_STORY_COMMENT, {
      "user_id": comment.user!.id,
      "comment": comment.comment,
      "story_id": comment.storyId,
  ...
  reset(String storyId) async {
    currentPage = 0;
    currentStory = convertNumber(storyId);
    maxPages = 0;
    list = [];
    isLoading = false;
  ...
  Future<bool> load() async {
    ...
    var response = await apiService
        .getRequest(JApi.GET_STORY_COMMENTS + "/${currentStory}");
    isLoading = false;
    list = [];
    notifyListeners();
    if (response != null) {
      if (response.length > 0) {
        ...
        for (int i = 0; i < response['data'].length; i++) {
          list.add(StoryComment.fromJson(response['data'][i]));
        }
  ...
  Future<bool> loadMoreData() async {
    ...
    var response = await JApiService().getRequest(
        JApi.GET_STORY_COMMENTS + "/${currentStory}" + "?page=${page}");
    refreshController.footerMode!.value = LoadStatus.idle;
    // context.read<PostProvider>().isLoading = false;
    if (response != null) {
      List<StoryComment> moreList = [];
      for (int i = 0; i < response['data'].length; i++) {
        moreList.add(StoryComment.fromJson(response['data'][i]));
      }
      currentPage = response['current_page'];
      maxPages = response['last_page'];
      addMore(moreList);
  ...
  addMore(List<StoryComment> moreList) {
  ...
  Future<int> editComment(commentId, comment) async {
    ...
    var response = await apiService.postRequest(JApi.EDIT_STORY_COMMENT,
        {"story_comment_id": commentId.toString(), "comment": comment});
    // isSaving = false;
    if (response != null) {
      try {
        if (response["edited"] == 1) {
          ...
          if (index > -1) {
            StoryComment newComment = _list[index];
  ...
  Future<int> deleteComment(StoryComment comment) async {
    JApiService apiService = JApiService();
    // isSaving = true;
    // int index = getStoryIndexById(storyId);
    // setSaved(index, !_list[index].isSaved);
    var response = await apiService.postRequest(JApi.DELETE_STORY_COMMENT, {
      "story_comment_id": comment.id.toString(),
    });
    // isSaving = false;
    if (response != null) {
      try {
        if (response["deleted"] == 1) {
          // int index = list.indexWhere((element) => element.id.toString() == comment.id.toString());
...
//   REPLIES
  ...
  Future<bool> loadCommentReplies(commentId) async {
    ...
    notifyListeners();
    var response = await apiService
        .getRequest(JApi.GET_STORY_REPLY_COMMENTS + "/${commentId}");
    ...
    if (response != null) {
      if (response.length > 0) {
        _list[index].replies = [];
        for (int i = 0; i < response['data'].length; i++) {
          _list[index].replies.add(StoryComment.fromJson(response['data'][i]));
        }
  ...
  Future<bool> loadMoreRepliesData(commentId) async {
    ...
    notifyListeners();
    var response = await JApiService().getRequest(
        JApi.GET_STORY_REPLY_COMMENTS + "/${commentId}" + "?page=${page}");
    _list[index].isReplyLoading = false;
    notifyListeners();
    // context.read<PostProvider>().isLoading = false;
    if (response != null) {
  ...
  Future<int> commentLike(commentId, String? parentCommentId) async {
    ...
    var response =
        await apiService.getRequest(JApi.LIKE_STORY_COMMENTS + "/$commentId");
    // isSaving = false;
    if (response != null) {
      try {
        if (response["liked"] == 1) {
