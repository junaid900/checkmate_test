...
import '../../modals/saved_story.dart';

class SavedStoryProvider extends ChangeNotifier{
  RefreshController refreshController = RefreshController();
  int _maxPages = 0;
  int _currentPage = 0;
  bool _isLoading = false;
  List<SavedStory> _list = [];
  ...
  List<SavedStory> get list => _list;
  set list(List<SavedStory> value) {
    _list = value;
    notifyListeners();
  }
  reset(user_id) async {
  ...
  Future<bool> load(user_id) async {
    ...
    isLoading = true;
    var response = await apiService.getRequest(JApi.GET_SAVED_STORIES+"/${user_id}");
    isLoading = false;
    list = [];
    notifyListeners();
    if(response  != null){
      if(response.length > 0){
        ...
        for(int i = 0 ; i <  response['data'].length ; i++ ){
          list.add(SavedStory.fromJson(response['data'][i]));
        }
  ...
  Future<bool> loadMoreData(user_id) async {
    ...
    var response = await JApiService().getRequest(
        JApi.GET_SAVED_STORIES +"/${user_id}" + "?page=${page}");
    refreshController.footerMode!.value = LoadStatus.idle;
    // context.read<PostProvider>().isLoading = false;
    if (response != null) {
      List<SavedStory> moreList = [];
      for (int i = 0; i < response['data'].length; i++) {
        moreList.add(SavedStory.fromJson(response['data'][i]));
      }
      currentPage = response['current_page'];
      maxPages = response['last_page'];
      addMore(moreList);
  ...
  addMore(List<SavedStory> moreList){
